...
@ISA = qw(Exporter);
@EXPORT = qw(read_passwd);
@EXPORT_OK = qw(ReadPasswd);
...
The B<read_passwd> function is almost same as
B<Term::ReadPassword::read_password> function.

The first and second parameters are just same as B<read_password>.
The first one is a prompt message, and the second one is timeout value.

The third parameter is different from B<read_password>.
